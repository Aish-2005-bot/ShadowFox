...
public class StudentInformation extends Application {
    ...
    public void start(Stage stage) throws Exception {
               ...
               Text title=new Text("Student Information System");
               title.setStyle(" -fx-font-size: 24px ; -fx-font-weight:bold");
               title.setFill(Color.WHITE);
               DropShadow ds=new DropShadow();
               ds.setColor(Color.PURPLE);
               title.setEffect(ds);VBox vb=new VBox(40,title);
               vb.setAlignment(Pos.CENTER);
                ...
                button1.setOnAction( e ->{
                    if(validInputs(rollnoField,nameTextField,courseField,emailField)){
                        studentlist.add(new Student(rollnoField.getText(),nameTextField.getText(),courseField.getText(),emailField.getText())
                        );
...
tableView.getSelectionModel().selectedItemProperty().addListener((obs, oldSel, newSel) -> {
    try {
        ...
        VBox vb1=new VBox(40,vb,hb1,hb,tableView);
        vb1.setStyle("-fx-background-color:plum");
